...
def run_label_propagation(X, y, y_true=None, description=""):
    ...
    # ------------------ APPLY SMOTE ON LABELED SAMPLES ONLY -------------------
    labeled_indices = np.where(y_train != -1)[0]
    X_labeled = X_train[labeled_indices]
    y_labeled = y_train[labeled_indices]

    print(f"Before SMOTE - Class distribution: {np.bincount(y_labeled)}")

    smote = SMOTE(random_state=42)
    X_labeled_resampled, y_labeled_resampled = smote.fit_resample(X_labeled, y_labeled)

    print(f"After SMOTE - Class distribution: {np.bincount(y_labeled_resampled)}")

    # Combine resampled labeled data with unlabeled
    X_train_combined = np.vstack((X_train[y_train == -1], X_labeled_resampled))
    y_train_combined = np.concatenate((np.full(X_train[y_train == -1].shape[0], -1), y_labeled_resampled))
...
if __name__ == "__main__":
    json_dir = "/scratch/data_temp"  # Update this path as needed
    main(json_dir)
