...
class TextToMel:
    ...
    def generate_mel_custom_durations(self, text, scaling_factors: dict, noise_scale=0.667):
        words = text.strip().split()
        mels = []

        for word in words:
            length_scale = scaling_factors.get(word, 1.0)
            print(f"Word: '{word}', Length scale: {length_scale}")

            symbols = list(self.hps.data.punc) + list(self.hps.data.chars)
            cleaner = self.hps.data.text_cleaners

            if getattr(self.hps.data, "add_blank", False):
                text_norm = text_to_sequence(word, symbols, cleaner)
                text_norm = commons.intersperse(text_norm, len(symbols))
            else:
                word_proc = " " + word.strip() + " "
                text_norm = text_to_sequence(word_proc, symbols, cleaner)
            ...
            with torch.no_grad():
                (y_gen_tst, *_), *_, _ = self.glow_tts_model(
                    x_tst,
                    x_tst_lengths,
                    gen=True,
                    noise_scale=noise_scale,
            ...
            mels.append(y_gen_tst.cpu())

        combined_mel = torch.cat(mels, dim=2).numpy()
        return combined_mel
...
class MelToWav:
    ...
    def generate_wav(self, mel):
        ...
        # Note: No pitch or energy modification happens here.
        # All prosody (pitch, energy, duration) is embedded in the mel.
...
if __name__ == "__main__":
    ...
    # Custom durations per word
    scaling_factors = {
        "pitch_k": p_k,
        "duration_k": k,
    }

    mel = text_to_mel.generate_mel_custom_durations(args.text, scaling_factors, args.noise_scale)
