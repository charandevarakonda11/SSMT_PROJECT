...
def pitch_loss(pred_pitch, target_pitch, mask=None):
    """Calculates the pitch loss (L1 loss)."""
    if mask is not None:
        return F.l1_loss(pred_pitch * mask, target_pitch * mask)
    else:
        mask = mask.unsqueeze(1)
        return F.l1_loss(pred_pitch, target_pitch)


def energy_loss(pred_energy, target_energy, mask=None):
    """Calculates the energy loss (L1 loss)."""
    if mask is not None:
        return F.l1_loss(pred_energy * mask, target_energy * mask)
    else:
